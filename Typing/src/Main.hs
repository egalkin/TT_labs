...
buildTypeUnificator (ApplicationTree p q) usedVarsNames clojMap  = 
        ...
        (UnificatorPair 
            ...
            (VarStatement newVarName),  (newUsedVars, snd $ snd qResult))
buildTypeUnificator (LambdaTree (Var x) p) usedVarsNames clojMap = 
        let newUsedVars = Set.insert x usedVarsNames
            pResult     = buildTypeUnificator p newUsedVars clojMap
            udpMap      = snd $ snd pResult
            varType     = Map.lookup x udpMap
        in unpackType varType pResult
        where 
            unpackType (Just varType) pResult = (UnificatorPair (getContext $ fst pResult) (ImplicationStatement (VarStatement varType) (getVarType $ fst pResult)),  
                (fst $ snd pResult, Map.delete x (snd $ snd pResult)))
            unpackType Nothing pResult        =
                    let udpUsedVars = Set.insert x (fst $ snd pResult)
                        newVarName  = setNewName ("a" ++ x) udpUsedVars
                    in 
                    (UnificatorPair (getContext $ fst pResult) (ImplicationStatement (VarStatement newVarName) (getVarType $ fst pResult)),
                        (udpUsedVars, Map.insert x newVarName (snd $ snd pResult)))
...
buildExprTypeTemplate (LambdaTree (Var x) p) usedVarsNames clojMap = 
        let newUsedVars = Set.insert x usedVarsNames
            pResult     = buildExprTypeTemplate p newUsedVars clojMap
            udpMap      = snd $ snd pResult
            varType     = Map.lookup x udpMap
        in 
        unpackType varType pResult
        where 
            unpackType (Just varType) pResult = (ImplicationStatement (VarStatement varType) (fst pResult),  (fst $ snd pResult, Map.delete x (snd $ snd pResult)))
            unpackType Nothing pResult        =
                    let udpUsedVars = Set.insert x (fst $ snd pResult)
                        newVarName  = setNewName ("a" ++ x) udpUsedVars
                    in 
                    (ImplicationStatement (VarStatement newVarName) (fst pResult),
                        (udpUsedVars, Map.insert x newVarName (snd $ snd pResult)))

getStatementLeftPart :: Statement -> Statement
getStatementLeftPart (ImplicationStatement lp rp) = lp
...
buildTypeInference expr@(LambdaTree var@(Var x) p) vtMap tiContext innerLevel usedVarsNames clojMap = 
    let newUsedVars      = Set.insert x usedVarsNames
        ...
        varTypeStatement = getStatementLeftPart $ fst typeTemplate 
...
getFreeVars (LambdaTree (Var x) p) usedVarsNames clojMap = 
    let newUsedVars = Set.insert x usedVarsNames
        pResult     = getFreeVars p newUsedVars clojMap
        udpMap      = snd $ snd pResult
        varType     = Map.lookup x udpMap
    in 
    unpackType varType pResult
    where 
        unpackType (Just varType) pResult = (Set.delete (x, varType) (fst pResult),  (fst $ snd pResult, snd $ snd pResult))
        unpackType Nothing pResult        =
                let udpUsedVars = Set.insert x (fst $ snd pResult)
                    newVarName  = setNewName ("a" ++ x) udpUsedVars
                in 
                (fst pResult, (fst $ snd pResult, snd $ snd pResult))
getFreeVars (ApplicationTree p q) usedVarsNames clojMap =
    ...
    in 
    (Set.union (fst pVars) (fst qVars), (fst $ snd qVars, snd $ snd qVars))
